...
const router = AutoRouter<IRequest, [env: Env, ctx: ExecutionContext]>({
  ...
  // Clear room data (for fixing CLIENT_TOO_OLD errors from old incompatible data)
  .delete("/api/rooms/:roomId", async (request, env) => {
    const id = env.TLDRAW_DURABLE_OBJECT.idFromName(request.params.roomId);
    const room = env.TLDRAW_DURABLE_OBJECT.get(id);
    // Construct a request to the durable object with /delete path
    const deleteUrl = new URL(request.url);
    deleteUrl.pathname = "/delete";
    const response = await room.fetch(
      new Request(deleteUrl.toString(), { method: "POST" })
    );
